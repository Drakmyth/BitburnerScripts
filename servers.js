...
export async function main(ns) {
    ...
    if (current_ram === next_upgrade) {
        const upgrade_cost = ns.getPurchasedServerCost(next_upgrade * 2) * ns.getPurchasedServerLimit();
        const formatted_cost = ns.nFormat(upgrade_cost, `($0.000a)`)
        ns.tprint(`Next upgrade (${next_upgrade * 2}) at ${formatted_cost}`);
        return;
    } else if (simulate) {
        const upgrade_cost = ns.getPurchasedServerCost(next_upgrade) * ns.getPurchasedServerLimit();
        const formatted_cost = ns.nFormat(upgrade_cost, `($0.000a)`)
